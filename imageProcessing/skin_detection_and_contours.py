...
cv2.createTrackbar("LS", "Tracking",48,255,nothing) #lower saturation = 0
cv2.createTrackbar("LV", "Tracking",0,255,nothing) #lower value = 110
cv2.createTrackbar("UH", "Tracking",50,255,nothing) #upper hue = 33
...
while True: # a loop that goes on unless esc key is encountered
    ...
    contours, heirarchy = cv2.findContours(thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE) #finding contours
    ...
    cv2.drawContours(res, contours, -1, [255,0,255], 2) #drawing contours on the final skin detection
